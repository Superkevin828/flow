...
class APIClient {
  constructor() {
    // FIX: also match 127.0.0.1 as localhost
  ...
  async request(endpoint, options = {}) {
    ...
    try {
      ...
      if (error.name === 'TypeError' && error.message.includes('fetch')) {
        throw new APIError('Cannot connect to server. Is the backend running on port 5000?', 0);
      }
  ...
  // Reports
  async getSummary() { return this.request('/reports/summary'); }
  async getMonthlyReport(month) { return this.request(`/reports/monthly?month=${month}`); }
  async getAnnualReport(year) { return this.request(`/reports/annual?year=${year}`); }

  // AI
  async generateForecast() { return this.request('/ai/forecast', { method: 'POST' }); }
  async generateInsights() { return this.request('/ai/insights', { method: 'POST' }); }
  ...
  async markInvoicePaid(id) {
    return this.request(`/invoices/${id}/mark-paid`, { method: 'POST' });
  }
  ...
  async initiateSubscription(billingCycle) {
    return this.request('/subscriptions/initiate', { method: 'POST', body: JSON.stringify({ billingCycle }) });
  }
}
